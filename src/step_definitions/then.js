...
import isNotExist from '../support/check/isNotExist';
...
Then(
    /^I expect that the title is( not)*( containing)* "([^"]*)?"$/,
    checkTitle
);

Then(
    /^I expect that element "([^"]*)?" exists(?: (\d+) times)*$/,
    checkElementCount
);
...
Then(
    /^I expect that the( css)* attribute "([^"]*)?" from element "([^"]*)?" is( not)*( containing)* "([^"]*)?"$/,
    checkProperty
);
...
Then(
    /^I expect that the (width|height) of element "([^"]*)?" is( not)* ([\d]+) pixels$/,
    checkDimension
);
