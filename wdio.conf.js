...
function getTestName(context) {
    var startIndex = context.meta.url.lastIndexOf("/");
    var endIndex = context.meta.url.lastIndexOf(".");
    var testName = "testPage";

    if(startIndex != -1 && endIndex != -1 && startIndex < endIndex) {
        testName = context.meta.url.substring(startIndex, endIndex);
    } else if (context.meta.url.endsWith("/")) {
        testName = "homePage";
    }
...
exports.config = {
    ...
    capabilities: [{
        ...
        //
        browserName: 'chrome',
        acceptInsecureCerts: true, // Must have Chrome 65+ for this option or you will get SSL errors internal network
        chromeOptions: {
    ...
    baseUrl: 'http://www-svt.nbnco.net.au',
    //
    ...
    visualRegression: {
        compare: new VisualRegressionCompare.LocalCompare({
            referenceName: getScreenshotName(path.join(process.cwd(), 'reports/screensCompare/reference')),
            screenshotName: getScreenshotName(path.join(process.cwd(), 'reports/screensCompare/screen')),
            diffName: getScreenshotName(path.join(process.cwd(), 'reports/screensCompare/diff')),
    ...
    // Jonno: driver config are copied from: https://github.com/vvo/selenium-standalone/blob/master/lib/default-config.js
    // I have commented out firefox driver because nbn proxy throws error since Selenium Standalone does not use the same proxy settings
    seleniumInstallArgs : {
        drivers: {
            chrome: {
                version: '2.37',
